...
class CycleRigidityBoundaryCertificate:
    theorem_id: str
    status: str
    required_count: int
    present_count: int
...
def cycle_rigidity_boundary_certificate(root: Path | str, required: Sequence[str], boundary_text: str) -> CycleRigidityBoundaryCertificate:
    normalized = _normalize_required(required)
    ...
    return CycleRigidityBoundaryCertificate(
        theorem_id="CLR-CBC-1",
